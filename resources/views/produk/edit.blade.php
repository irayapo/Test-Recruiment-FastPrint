    ...
    <section class="bg-white dark:bg-gray-900 p-5 sm:p-8">
        <div class="mx-auto max-w-screen-xl px-4 lg:px-16">
            <div class="bg-white dark:bg-gray-800 sm:rounded-lg shadow-lg p-6">
                ...
                <!-- Form Edit Produk -->
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-6">Edit Produk</h2>
                            ...
                            <select id="kategori_id" name="kategori_id"
                                ...
                                @foreach ($kategoris as $kategori)
                                    <option value="{{ $kategori->id_kategori }}"
                                        {{ $product->kategori_id == $kategori->id_kategori ? 'selected' : '' }}>
                            ...
                            <select id="status_id" name="status_id"
                                ...
                                @foreach ($statuses as $status)
                                    <option value="{{ $status->id_status }}"
                                        {{ $product->status_id == $status->id_status ? 'selected' : '' }}>
